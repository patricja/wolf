...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
        $file1 = WOLF_SITE_PATH . $classFile;
        $file2 = WOLF_INSTALL_PATH . $classFile;
        if(is_file($file1)) {
                require_once($file1);
...
function exceptionHandler($e) {
  echo "Wolf: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlEnt($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CWolf::Instance()->config['character_encoding']);
}
